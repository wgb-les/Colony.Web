define(['jquery'], function () {
	...
	function isAuthenticatedInternal() {
		$.ajax({
			type: 'POST',
			url: rootUrl + "/members/authentication/getloggedinuserajax?isBacked=true",
			success: function (data) {
				if (data.success) {
	...
	function logOutInternal() {
		$.ajax({
			type: 'POST',
			url: rootUrl + "/members/authentication/logoutajax?isBacked=true",
			async: false,
			success: function (data) {
				if (data.success) {
					_private.currentUser = null;
	...
	function loginInternal(formData) {
		var loggedIn = false;
		$.ajax({
		    url: rootUrl + "/members/authentication/LoginAjax?isBacked=true",
			method: 'POST',
			async: false,
			data: formData, 
			success: function (data) {
	...
	Context.formatDate = function (strDate) {
	    if (strDate.indexOf('T') > 0)
	        strDate = strDate.substring(0, strDate.indexOf('T'));
	    return $.datepicker.formatDate('dd/mm/yy', $.datepicker.parseDate('yy-mm-dd', strDate));
	}
