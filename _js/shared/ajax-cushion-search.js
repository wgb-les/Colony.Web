    ...
    var search = {
        ...
        postRender: function () {
            ...
            setTimeout(function () {
                ...
                filterForm.on('click', '.filterFRTreated .clearFilter', function () { self.clearSelectedCheck('.filterFRTreated'); self.execute(true); return false; });
                filterForm.on('click', '.filterFRInherent .clearFilter', function () { self.clearSelectedCheck('.filterFRInherent'); self.execute(true); return false; });
                filterForm.on('click', '.filterFRTreatable .clearFilter', function () { self.clearSelectedCheck('.filterFRTreatable'); self.execute(true); return false; });
                ...
                filterForm.on('click', '.filterRoom .clearFilter', function () { self.clearSelectedCheck('.filterRoom'); self.execute(true); return false; });
                ...
                filterForm.on('click', '.clearAllFilters', function () {
                    self.clearSelectedCheck('.filterColours,.filterBrands,.filterPattern,.filterEndUse,.filterType,.filterProductGroup,.filterFlameRetardant,.filterUsage,.filterComposition,.filterFRTreated,.filterFRTreatable,.filterFRInherent,.filterRoom');
                    self.toggleClearText('input[name="keywords"]');
                    self.removeAllOptions('select[name="keywords"]'); //If 'fcbkcomplete' is attached. The text input is switched to a select. Remove all its childen.
                    self.setSearchTabUrls();
                    self.execute(true);
        ...
        execute: function (newSearch, callback) {
            ...
            var categoryName = this.getQueryString('categoryName');
            var categoryId = this.getQueryString('categoryId');
            var productGroups = this.getQueryString('SelectedProductGroups');
            var ajaxURL = '/shops/wgshopsearch/AjaxCushion/?page=' + this.page + '&perPage=120'; //if per page changes, change the if below.
          ...
          //  var selectedProductCategories = $(".filterProductGroup input:checked");
            ...
            selectedProductCategoriesId = 6508; //selectedProductCategories.first().val() || this.getQueryString('SelectedProductGroups');
            ...
            if (selectedProductCategoriesId && this.lastBannerCategoryIdUsed !== selectedProductCategoriesId) { // && (!this.lastBannerCategoryIdUsed || this.lastBannerCategoryIdUsed !== selectedProductCategoriesId)
                ...
                var currentRequest = $.ajax({
                    ...
                    type: 'GET',
                    data: {
                        skuAttributeOptionId: 6508 //selectedProductCategoriesId
                    },
                    complete: function () {
        ...
        init: function (formSelector) {
            ...
            $.when(self.loadTemplate('search-results'),
                self.loadTemplate('cushion-result-grid'),
                self.loadTemplate('cushion-filter'),
                self.loadTemplate('cushion-result-list'))
